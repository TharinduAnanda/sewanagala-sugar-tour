...
import { 
  ...
  FaCheckCircle, FaClock, FaTimesCircle, FaUsers, FaPlus, FaStar, FaTimes
} from 'react-icons/fa'
...
function AdminCalendarPageContent() {
  ...
  const [selectedClosure, setSelectedClosure] = useState<FactoryClosure | null>(null)
  ...
  const fetchFactoryClosures = async () => {
    try {
      ...
      const response = await axios.get(`/api/calendar/closures?start=${startDate}&end=${endDate}`)
      
      if (response.data) {
        setFactoryClosures(Array.isArray(response.data) ? response.data : [])
      }
    } catch (err) {
      console.error('Failed to fetch factory closures:', err)
      setFactoryClosures([])
    }
  }
  ...
  const handleAddClosure = async () => {
    try {
      ...
      if (response.ok) {
        ...
        fetchFactoryClosures()
        alert('Factory closure added successfully')
      } else {
        alert('Failed to add factory closure')
      }
    } catch (error) {
      console.error('Error adding closure:', error)
      alert('Error adding factory closure')
    }
  }

  const handleDeleteClosure = async (id: number) => {
    try {
      if (!confirm('Are you sure you want to delete this factory closure?')) {
        return
      }

      const response = await fetchWithAuth(`/api/calendar/closures/${id}`, {
        method: 'DELETE'
      })
      ...
      if (response.ok) {
        ...
        alert('Factory closure deleted successfully')
      } else {
        alert('Failed to delete factory closure')
      }
    } catch (error) {
      console.error('Error deleting closure:', error)
      alert('Error deleting factory closure')
    }
  }
  ...
  const getClosureForDate = (date: Date) => {
    const dateStr = getDateString(date)
    return factoryClosures.find(c => c.date === dateStr)
  }
  ...
  const handleDayClick = (date: Date) => {
    ...
    setSelectedClosure(getClosureForDate(date) || null)
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Add Factory Closure Button - Top Right */}
          ...
          <Dialog open={isAddingClosure} onOpenChange={setIsAddingClosure}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Add a date when the factory will be closed for maintenance, holiday, or other reasons.
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                ...
                <Button onClick={handleAddClosure} className="bg-green-600 hover:bg-green-700">
                  Add Closure
                </Button>
              </DialogFooter>
            </DialogContent>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-7 gap-2">
                  {days.map((day, index) => {
                    ...
                    const closure = getClosureForDate(day)
                    ...
                    return (
                      <motion.div
                        ...
                        className={`
                          ...
                          ${closure ? 'bg-orange-50 border-orange-300' : 
                            holiday ? 'bg-red-50 border-red-300' : 
                            (dayBookings.length > 0 ? 'bg-green-50' : 'bg-white')}
                          hover:shadow-md
                        `}
                        title={
                          closure ? `Factory Closed: ${closure.reason}` :
                      ...
                      >
                        <div className="text-sm font-semibold text-gray-900 flex items-center justify-between">
                          ...
                          {closure && <FaTimes className="text-orange-600 text-xs" />}
                        </div>
                        {closure && (
                          <div className="mt-1">
                        ...
                        {!closure && !holiday && dayBookings.length > 0 && (
              ...
              <CardContent>
                {selectedDate ? (
                  <div className="space-y-4">
                      ...
                      <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-4">
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-orange-900">?? Factory Closed</span>
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleDeleteClosure(selectedClosure.id)}
                        ...
                        <div className="space-y-2">
                          <div className="text-sm">
                            ...
                            <span className="ml-2 capitalize px-2 py-1 bg-orange-100 text-orange-800 rounded text-xs">
                              {selectedClosure.type}
                            </span>
                          </div>
                          <div className="text-sm">
                            <span className="font-medium">Reason:</span>
                            <p className="text-orange-700 mt-1">{selectedClosure.reason}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {selectedDayBookings.length === 0 && !selectedHoliday && !selectedClosure ? (
            ...
            <Card className="mt-4">
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">?? Factory Closures</span>
                    <span className="font-bold text-lg text-orange-600">
                      {factoryClosures.length}
                    </span>
                  </div>
                </div>
              </CardContent>
...
export default function AdminCalendarPage() {
  ...
  return (
    <AdminLayout
      ...
      description="Manage factory closures and public holidays"
